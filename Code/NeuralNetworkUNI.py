...
for epoch in range(n_epochs):
    ...
    for patient_index, slide in enumerate(MIL_DataLoader.train_dataloader_uni):
    ...
    with torch.no_grad():
        for patient_index, slide in enumerate(MIL_DataLoader.test_dataloader_uni):    
            patient_features = slide['features'].to(device)
            patient_Target = slide['t_cells'].to(device).to(torch.float32)
    ...
    save_checkpoint(checkpoint, filename="UMaxGraphcheckpoint.pth.tar", train_losses=epochAverageLossTrain,test_losses=epochAverageLossTest, epoch=(epoch+1))
    ...
    if epochAverageLossTest < best_loss:
        ...
        early_stop_counter = 0
        save_checkpoint(best_state, filename="UMaxGraphbest.pth.tar", train_losses=epochAverageLossTrain, test_losses=epochAverageLossTest)
