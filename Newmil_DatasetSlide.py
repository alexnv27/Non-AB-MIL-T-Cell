...
class NewmilDataset(Dataset):
    ...
    def __getitem__(self, idx):
        df_row = self.df.iloc[idx] #added iloc

        try:
            T_cells = df_row['T_cells'] #deleted iloc and [0]
          
        except Exception as e:
            #print(f"Error accessing T_cells: {e}")
            T_cells = None  # or you can handle it in another way

        patient_ID=df_row['ID'] #deleted iloc and [0]
        slide_path=df_row['file_path'] #deleted iloc and [0]
        features = []
        coords = []
        paths=[]
        '''
