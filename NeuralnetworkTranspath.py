...
for epoch in range(n_epochs):
    ...
    for patient_index, slide in enumerate( MIL_DataLoader.train_dataloader_transpath):
    ...
    with torch.no_grad():
        for patient_index, slide in enumerate( MIL_DataLoader.test_dataloader_transpath): 
    ...
    save_checkpoint(checkpoint, filename="TMaxGraphcheckpoint.pth.tar", train_losses=epochAverageLossTrain,test_losses=epochAverageLossTest, epoch=(epoch+1))
    ...
    if epochAverageLossTest < best_loss:
        ...
        early_stop_counter = 0
        save_checkpoint(best_state, filename="TMaxGraphbest.pth.tar", train_losses=epochAverageLossTrain, test_losses=epochAverageLossTest)
